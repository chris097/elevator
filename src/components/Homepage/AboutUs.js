    ...
    return (
        <div className='w-[90%] mx-auto pt-32'>
            <div className='flex'>
                ...
                <div className=' bg-primaryyellow w-[606px] lg:py-20 py-10 px-10'>
                    <Reveal>
                         <p className='text-primarypink text-xl font-semibold uppercase'>ABOUT US</p>
                    <p className='text-[38px] font-semibold lg:mt-10 mt-6'>WE ARE QUALIFIED IN EVERY WORKING DEPARTMENTS</p>
                   </Reveal>
                    <p className='lg:text-[24px] text-xl text-[#6E777D] mt-4 mb-7'>Laoreet conubia parturient lacinia pulvinar senectus vel posuere proin nec montes dapibus, elementum blandit sem et massa ornare habitasse cursus erat nascetur, a sed placerat sagittis euismod.</p>
                    {abouts.map((about, index) => (
                        <div key={index} className='flex gap-2 items-center mt-6'>
                        <svg className='w-[18px] h-[18px] lg:w-[21px] lg:h-[21px]' viewBox="0 0 20 21" fill="none" xmlns="http://www.w3.org/2000/svg">
                            ...
                            <span className='text-[#6E777D] lg:text-xl text-base'>{about.description}</span>
                    </div>
                    ))}
                    <button className='bg-black w-2/4 h-16 rounded-sm text-xl font-semibold mt-16 text-white'>Learn More</button>
                </div>
            </div>
        </div>
    );
