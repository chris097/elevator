...
const Result = () => {
    return (
        <div className='relative'>
            <div className=''>
                <div className='bg-black absolute opacity-90 w-1/2 z-30 lg:h-[800px] h-[650px]' />
                <div className='bg-black absolute z-20 w-1/2 lg:h-[800px] h-[650px]' />
                <Image className="w-full lg:h-[800px] h-[650px] image-container" src={result} alt="elevator result" />
            </div>
            <div className='text-white z-40 absolute top-0 px-10 pt-20'>
                <Reveal><p className='lg:text-xl text-base text-primarypink'>LATEST PROJECTS</p></Reveal>
                <Reveal><p className='lg:text-[52px] text-2xl font-semibold leading-snug text-white lg:w-1/2 mt-5'>EXCELLENCE DRIVEN FOR GETTING UNRIVALED RESULTS.</p></Reveal>
                    <div className='grid lg:grid-cols-4 grid-cols-2 mt-10 lg:gap-6 gap-3'>
                {results.map((result, index) => (
                    <Reveal  key={index}>
                        <div className='bg-primarypink w-full rounded-md lg:h-[280px] lg:py-0 py-4 h-auto gap-1 flex justify-center flex-col items-center'>
                        <svg className='lg:w-[80px] lg:h-[80px] w-[40px] h-[40px]' viewBox="0 0 78 80" fill="none" xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink">
                            ...
                            <p className='text-white lg:text-[40px] md:mt-4 mt-2 font-bold text-2xl'>{result.number}</p>
                            <p className='lg:text-xl text-sm md:mt-4 mt-2 font-bold text-white'>{result.title}</p>
                    </div>
                    </Reveal>
                ))}
                </div>
